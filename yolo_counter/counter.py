...
def _images_dir(base: str) -> str:
    """Directory holding raw/collected dataset images.

    Must be named "images" (not "raw") because ultralytics locates a
    label file by swapping the "images" path segment for "labels" — any
    other folder name means it silently finds zero labels. Transparently
    migrates datasets created before this rename.
    """
    img_dir = os.path.join(base, "dataset", "images")
    old_dir = os.path.join(base, "dataset", "raw")
    if not os.path.isdir(img_dir) and os.path.isdir(old_dir):
        os.rename(old_dir, img_dir)
    return img_dir


@app.post("/collect/{cam_id}")
def collect_frames(cam_id: str, frames: int = 50, stream: str = "", interval: float = 5.0):
    """Capture N frames (one every `interval` seconds) and save to dataset/images/.
...
def dataset_image(filename: str):
    base = os.path.dirname(sys.executable) if getattr(sys, 'frozen', False) else os.path.dirname(os.path.abspath(__file__))
    path = os.path.join(_images_dir(base), filename)
